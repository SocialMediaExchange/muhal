...
export default class Home extends Component {
  static async getInitialProps(ctx) {
    ...
    if (isServer) {
      ...
      const res = await fetch("/_data/cases", {
  ...
  onSelect(label) {
    ...
    return function(option) {
      component.setState({
        [label]:
          option.target.value === "لا شيء محدد" ? null : option.target.value
      });
    };
  }
  ...
  render() {
    ...
    const mediumOptions = uniq(
      filter(notEmpty, cases.map(prop("وسيلة النشر")))
    ).sort();
    const yearOptions = uniq(filter(notEmpty, cases.map(prop("Year")))).sort();
    const complaintOptions = uniq(
      filter(notEmpty, cases.map(prop("Complaint_ar")))
    ).sort();
    ...
    return (
      <Layout>
        <div
          className="flex flex-wrap mt2 center"
        ...
        >
          ...
          <Select
            ...
            onChange={this.onSelect("Medium")}
            selectLabel="وسيلة النشر"
          />
          <Select
            ar
            options={yearOptions}
            selected={this.state["Year"]}
            onChange={this.onSelect("Year")}
            selectLabel="السنة"
          />
          <Select
            ar
            options={complaintOptions}
            selected={this.state["Complaint"]}
            onChange={this.onSelect("Complaint")}
            selectLabel="الجهة المدعية"
          />
        </div>
        <div className="flex flex-wrap mt2 mw8 pa4 center"
        style={{ "background-color": "#ffd991" }}

        >
          {casesToDisplay.map(data => (
            <Card data={data} key={data.id} ar />
          ))}
        </div>
      </Layout>
    );
